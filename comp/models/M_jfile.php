...
class M_jfile extends CI_Model 
{

	//var $table = "files";
	var $column = array('JFileID, JFileName'); 
	var $order = array('JFileName' => 'ASC');  
	...
	*/
	function get_data($app=0)
	{
		...
		$this->db->select('JFileID, JFileName,AppKey');
		$this->db->from("jfiles");
		$this->db->select('JFileID, JFileName,AppName,AppKey')->join('Apps', 'Apps.AppID = jfiles.AppKey');
	...
	function count_filtered($ser=0)
	{
		...
		$this->db->select('JFileID');
		$this->db->from("jfiles");
		$this->db->order_by('JFileName', 'ASC');
		$query = $this->db->get();
