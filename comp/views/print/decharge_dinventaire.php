	...
	$pdf->Ln(1);
	$pdf->WriteHTMLCell(80,0,'','',$objets[0]->ReferenceName,0,1,1,true,'L',true);
	...
	$tab_famille = array();
	$data = array();
	//$countD = $countDesgination[0];
	foreach ($countDesgination as $line)
	{
		$ctr=0;
		$str='';
		$des = '';
		//echo $line->FamilleName.' - ';
		foreach ($objets as $row)
		{
			if($row->Desgination == $line->Desgination)
			{
		...
		array_push($data, array('06', $line->FkFamille, $str, '', $des, $ctr, 'Neuf', 'Neuf'));
	}
