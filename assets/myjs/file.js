		...
		$(document).ready(function()
		{
			...
			table = $("#table").DataTable({
			  ...
			  "ajax": {
				"url": base_url+"FileController/ajax_list/0",
				"type": "POST"
			  },
			  "columnDefs": [
				{ 
		...
		function edit(id)
		{
			...
			$.ajax({
				url : base_url+"FileController/edit/" + id,
				type: "POST",
				dataType: "JSON",
				success: function(data)
				{
                    $("#cb_apps2").prop('disabled', true);				
					$('[name="id"]').val(data.FileID);
		...
		function save()
		{
			...
			if(save_method == 'add')
			{
				url = base_url+"FileController/add/";
			} 
			else 
			{
				url = base_url+"FileController/ajax_update/";
				//save_method = 'add';
			}
		...
		function _delete(id)
      {
        if(confirm('Are you sure delete this data?'))
        {
          ...
          $.ajax({
              url : base_url+"FileController/ajax_delete/"+id, // +"/"+file
              type: "POST",
              dataType: "JSON",
              success: function(data)
              {
